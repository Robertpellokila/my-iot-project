...
async function onRequestPost(context2) {
  try {
    ...
    if (deviceKey !== env2.DEVICE_API_KEY) {
      return jsonResponse(
        {
          success: false,
    ...
    const normalizedUid = String(uid).replace(/[^a-fA-F0-9]/g, "").toUpperCase();
    if (!normalizedUid) {
      return jsonResponse(
        {
          success: false,
    ...
    const supabaseKey = env2.SUPABASE_SECRET_KEY;
    ...
    console.error("Function error:", error3);
    return jsonResponse(
      {
        success: false,
...
// ../.wrangler/tmp/pages-6Dfmnq/functionsRoutes-0.7021747918237482.mjs
var routes = [
  {
